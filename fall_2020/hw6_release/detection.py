...
def hog_feature(image, pixel_per_cell=8):
    ...
    """
    ### YOUR CODE HERE
    tup = (pixel_per_cell, pixel_per_cell)
    hog_feature, hog_image = feature.hog(image, pixels_per_cell = tup, block_norm='L1',visualize=True)
    ### END YOUR CODE
...
def sliding_window(image, template_feature, step_size, window_size, pixel_per_cell=8,
    ...
    for r in range(0, H, step_size):
        for c in range(0, W, step_size):
            score = 0
            ### YOUR CODE HERE
            img_feature, pic = hog_feature(pad_image[r: r + window_size[0], c:c + window_size[1]], pixel_per_cell)
            score = np.dot(img_feature, template_feature)
            if score > max_score:
                max_score = score
                maxr = r
                maxc = c
...
def pyramid_score(image, template_feature, shape, step_size=20,
    ...
    ### YOUR CODE HERE
    for i in images:
        (i_score, ir, ic, i_map_resized) = sliding_window(i[1], template_feature, step_size, shape, pixel_per_cell)
        if i_score >= max_score:
            max_score = i_score
            maxr = ir
            maxc = ic
            max_scale = i[0]
            max_response_map = i_map_resized
...
def compute_displacement(part_centers, face_shape):
    ...
    ### YOUR CODE HERE
    img_center = (face_shape[0] / 2, face_shape[1] / 2)
    for c in range(part_centers.shape[0]):
        x = img_center[0] - part_centers[c, 0]
        y = img_center[1] - part_centers[c, 1]
        d[c] = (x, y)
...
def gaussian_heatmap(heatmap_face, heatmaps, sigmas):
    ...
    ### YOUR CODE HERE
    heatmap = 0
    for m in range(len(heatmaps)):
        heatmap += gaussian(heatmaps[m], sigmas[m])
